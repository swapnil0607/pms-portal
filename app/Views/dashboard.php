...
<section class="home-workspace">
    ...
    <form method="post" action="/work-logs/create" class="panel home-log-form">
        ...
        <div class="compact-form-grid">
            <label>
                Client Name
                <input type="text" name="project_group" placeholder="Example: Jindal Stainless" required>
            </label>
            <label>
                Project Phase
                <input type="text" name="phase" placeholder="Example: Phase 1" required>
            </label>
            <label>
                Module Number & Name
                <input type="text" name="module_name" placeholder="Example: M01 - LMS Setup" required>
            </label>
            <label>
                Task Category
            ...
            <?php else: ?>
                <div class="simple-list">
                    <?php foreach ($myTasks as $task): ?>
                        <a class="simple-row" href="/tasks/show?id=<?= e((string) $task['id']) ?>">
                            <span>
                                <strong><?= e($task['title']) ?></strong>
                                <small><?= e($task['project_name']) ?><?= $task['due_date'] ? ' - Due ' . e($task['due_date']) : '' ?></small>
                            </span>
